...
def test_mean_and_std_particle_plot():
    ...
    segment = cheetah.Segment(
        elements=[
            ...
            cheetah.Drift(length=torch.tensor(1.0)),
            cheetah.Corrector(length=torch.tensor(0.3), name="V7SMATCH"),
            cheetah.Drift(length=torch.tensor(0.2)),
            cheetah.Corrector(length=torch.tensor(0.3), name="H10SMATCH"),
            cheetah.Drift(length=torch.tensor(7.0)),
            cheetah.Corrector(length=torch.tensor(0.3), name="H12SMATCH"),
