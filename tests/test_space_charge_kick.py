...
def test_space_charge_with_aperture_cutoff():
    """
    Tests that the space charge kick is correctly calculated only for the surviving
    particles when an aperture is used.
    """
    incoming_beam = cheetah.ParticleBeam.from_parameters(
        num_particles=torch.tensor(10_000),
        total_charge=torch.tensor(1e-9),
    ...
    drift1 = cheetah.Drift(length=torch.tensor(0.2))
    ...
    drift2 = cheetah.Drift(length=torch.tensor(0.25))
    space_charge = cheetah.SpaceChargeKick(effect_length=torch.tensor(0.5))
    segment = cheetah.Segment(elements=[drift1, aperture, drift2, space_charge, drift2])

    outgoing_beam_without_aperture = segment.track(incoming_beam)

    # Activate the aperture
    aperture.is_active = True
