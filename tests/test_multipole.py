...
from cheetah import Drift, ParticleBeam, Segment
...
def test_multipole_vs_quadrupole():
    ...
    multipole_quad = Multipole(
        ...
        polynom_b=torch.tensor([0.0, k1])  # B1 = k1
    )
...
def test_multipole_as_horizontal_corrector():
    """
    Test that a multipole with B0 coefficient behaves like a horizontal corrector element.
    """
    # Set parameters
...
def test_skew_multipole_behavior():
    """
    Test that skew multipole components (polynom_a) work as expected (like 90deg tilted normal quadrupole).
    """
...
def test_multipole_max_order():
    ...
    multipole_limited = Multipole(
        length=torch.tensor(1.0),
        polynom_b=torch.tensor([0.0, 1.0, 0.5, 0.2, 0.1, 0.05]),  # Only first 3 will be used
        max_order=2,
    )
...
# #     multipole = Multipole(
# #         length=torch.tensor(1.0),
# #         polynom_b=torch.tensor([0.0, 1.0]).unsqueeze(0).repeat(3, 1) * k1_values.unsqueeze(1)
# #     )
