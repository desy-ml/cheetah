...
def test_ocelot_to_particlebeam():
    parray = ocelot.astraBeam2particleArray("tests/resources/ACHIP_EA1_2021.1351.001")
    beam = lynx.ParticleBeam.from_ocelot(parray)
...
def test_ocelot_lattice_import():
    ...
    segment = lynx.Segment.from_ocelot(cell=cell)

    assert isinstance(segment.elements[0], lynx.Drift)
    assert isinstance(segment.elements[1], lynx.Quadrupole)
    assert isinstance(segment.elements[2], lynx.Drift)
