...
- There is a warning now when converting elements from Elegant or Bmad that have names which are invalid for use with the `segment.element_name` syntax, and add a convience method for explicitly converting these names to valid Python variable names. (see #411) (@amylizzle, @jank324)
