...
- Rework the `Aperture` element. Now `ParticleBeam` has a `survival_probabilities` attribute that keeps track of the lost particles. The statistical beam parameters are calculated only w.r.t. surviving particles. Note that the `Aperture` breaks differentiability if activated. (see #268) (@cr-xu, @jank324)
