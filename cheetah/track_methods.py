...
def base_tmatrix(
    ...
    T[..., 0, 0, 5] = (
        2 * -hx / 12 / beta * khk * (3 * sx * j1 - dx**2)
        + 0.5 * hx**2 / beta * sx**2
        + 0.25 / beta * k1 * length * sx
    )
    T[..., 0, 1, 5] = (
        2 * -hx / 12 / beta * khk * (sx * dx**2 - 2 * cx * j2)
    ...
    T[..., 2, 0, 2] = (
        2 * 0.5 * k2 * (cy * jc - 2 * k1 * sy * js) + 0.5 * hx * k1 * sx * sy
    )
    T[..., 2, 0, 3] = 2 * 0.5 * k2 * (sy * jc - 2 * cy * js) + 0.5 * hx * sx * cy
    T[..., 2, 1, 2] = (
        2 * 0.5 * k2 * (cy * js - 2 * k1 * sy * jd) + 0.5 * hx * k1 * dx * sy
    )
    T[..., 2, 1, 3] = 2 * 0.5 * k2 * (sy * js - 2 * cy * jd) + 0.5 * hx * dx * cy
    T[..., 2, 2, 5] = (
        2 * 0.5 * hx / beta * k2 * (cy * jd - 2 * k1 * sy * jf)
        + 0.5 * hx**2 / beta * k1 * j1 * sy
        - 0.25 / beta * k1 * length * sy
    )
    T[..., 2, 3, 5] = (
        2 * 0.5 * hx / beta * k2 * (sy * jd - 2 * cy * jf)
        + 0.5 * hx**2 / beta * j1 * cy
        - 0.25 / beta * (sy + length * cy)
    )
    T[..., 3, 0, 2] = (
        2 * 0.5 * k1 * k2 * (2 * cy * js - sy * jc) + 0.5 * (k2 + hx * k1) * sx * cy
    )
    T[..., 3, 0, 3] = (
        2 * 0.5 * k2 * (2 * k1 * sy * js - cy * jc) + 0.5 * (k2 + hx * k1) * sx * sy
    )
    T[..., 3, 1, 2] = (
        2 * 0.5 * k1 * k2 * (2 * cy * jd - sy * js) + 0.5 * (k2 + hx * k1) * dx * cy
    )
    T[..., 3, 1, 3] = (
        2 * 0.5 * k2 * (2 * k1 * sy * jd - cy * js) + 0.5 * (k2 + hx * k1) * dx * sy
    )
    T[..., 3, 2, 5] = (
        2 * 0.5 * hx / beta * k1 * k2 * (2 * cy * jf - sy * jd)
        + 0.5 * hx / beta * (k2 + hx * k1) * j1 * cy
        + 0.25 / beta * k1 * (sy - length * cy)
    )
    T[..., 3, 3, 5] = (
        2 * 0.5 * hx / beta * k2 * (2 * k1 * sy * jf - cy * jd)
        + 0.5 * hx / beta * (k2 + hx * k1) * j1 * sy
        - 0.25 / beta * k1 * length * sy
    )
    T[..., 4, 0, 0] = -1 * hx / 12 / beta * khk * (
        sx * dx + 3 * j1
    ...
    T[..., 4, 1, 1] = (
        -1 * hx / 6 / beta * khk * j2
        - 0.5 / beta * sx
        - 0.25 / beta * k1 * (j1 - sx * dx)
    )
    T[..., 4, 0, 5] = (
        -2 * hx**2 / 12 / beta**2 * khk * (3 * dx * j1 - 4 * j2)
        + 0.25 * hx / beta**2 * k1 * j1 * (1 + cx)
        + 0.5 * hx / beta**2 * igamma2 * sx
    )
    T[..., 4, 1, 5] = (
        -2 * hx**2 / 12 / beta**2 * khk * (dx * dx**2 - 2 * sx * j2)
        + 0.25 * hx / beta**2 * k1 * sx * j1
        + 0.5 * hx / beta**2 * igamma2 * dx
    )
    T[..., 4, 5, 5] = (
        -1 * hx**3 / 6 / beta**3 * khk * (3 * j3 - 2 * dx * j2)
