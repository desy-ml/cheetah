...
class Multipole(Element):
    ...
    :param fringe_quad_entrance: Whether to apply quadrupole fringe field at entrance (0=no fringe, 1=Lee-Whiting, 2=Lee-Whiting+Elegant).
    :param fringe_quad_exit: Whether to apply quadrupole fringe field at exit (0=no fringe, 1=Lee-Whiting, 2=Lee-Whiting+Elegant).
    :param fringe_int_m0: Fringe field integrals for entrance (I0m/K1, I1m/K1, I2m/K1, I3m/K1, Lambda2m/K1).
    :param fringe_int_p0: Fringe field integrals for exit (I0p/K1, I1p/K1, I2p/K1, I3p/K1, Lambda2p/K1).
    ...
    def __init__(
        ...
        device, dtype = verify_device_and_dtype(
            [length, polynom_a, polynom_b, misalignment, tilt, fringe_int_m0, fringe_int_p0], 
            device, dtype
        )
        ...
        if polynom_a is None:
            ...
            if polynom_a.size(0) <= max_order:
                ...
                padding = torch.zeros(max_order + 1 - polynom_a.size(0), **factory_kwargs)
        ...
        if polynom_b is None:
            ...
            if polynom_b.size(0) <= max_order:
                # Pad with zeros if needed
                padding = torch.zeros(max_order + 1 - polynom_b.size(0), **factory_kwargs)
    ...
    def _track_symplectic4(self, incoming: ParticleBeam) -> ParticleBeam:
        """
        Track particles through the multipole element using the 4th order symplectic integration.
    ...
    def _track_symplectic4_rad(self, incoming: ParticleBeam) -> ParticleBeam:
        """
        Track particles through the multipole element using the 4th order symplectic integration
        with radiation effects included.
    ...
    def _drift6(self, x: torch.Tensor, px: torch.Tensor, y: torch.Tensor, py: torch.Tensor, 
                tau: torch.Tensor, delta: torch.Tensor, length: float) -> None:
        """
        ...
        # In PyAT, r[5] (tau) += NormL*(r[1]*r[1]+r[3]*r[3])/(2*(1+r[4])) where r[4] is delta
        tau += norm_length * (px * px + py * py) / (2.0 * (1.0 + delta))

    def _apply_kick_rad(self, x: torch.Tensor, y: torch.Tensor, px: torch.Tensor, py: torch.Tensor, 
                     delta: torch.Tensor, kick_strength: float, energy: torch.Tensor) -> None:
        """
        ...
        delta.sub_(CRAD * (1.0 + delta)**2 * B2P * (1.0 + (xpr**2 + ypr**2)/2.0) * kick_strength)
    ...
    def _apply_kick(self, x: torch.Tensor, y: torch.Tensor, px: torch.Tensor, py: torch.Tensor, kick_strength: float) -> None:
        """
        Apply multipole kick to particles.
        
        This method calculates the magnetic field components from the multipole expansion and applies
        the resulting momentum kicks to the particles. The algorithm recursively computes the field
        components following the same approach as in PyAT.
    ...
    def is_skippable(self) -> bool:
        ...
        """
        return (torch.all(self.polynom_a == 0).item() and 
                torch.all(self.polynom_b == 0).item())
    ...
    def defining_features(self) -> list[str]:
        return super().defining_features + [
            "length", "polynom_a", "polynom_b", "max_order", 
            "misalignment", "tilt", "fringe_quad_entrance", "fringe_quad_exit",
            "fringe_int_m0", "fringe_int_p0"
        ]
    ...
    def _quad_fringe_pass_p(self, x: torch.Tensor, px: torch.Tensor, y: torch.Tensor, 
                          py: torch.Tensor, tau: torch.Tensor, delta: torch.Tensor, b2: float) -> None:
        """
    ...
    def _quad_fringe_pass_n(self, x: torch.Tensor, px: torch.Tensor, y: torch.Tensor, 
                          py: torch.Tensor, tau: torch.Tensor, delta: torch.Tensor, b2: float) -> None:
        """
    ...
    def _quad_partial_fringe_matrix(self, K1: torch.Tensor, in_fringe: float, 
                                   fringe_int: torch.Tensor, part: int) -> torch.Tensor:
        """
        ...
        if part == 1:
            ...
            K1_y = -K1
            J1y = in_fringe * (K1_y * fringe_int[1] - 2 * K1_y * K1_y * fringe_int[3] / 3.0)
            J2y = -J2x
            J3y = J3x
        else:  # part == 2
            J1x = in_fringe * (K1 * fringe_int[1] + K1sqr * fringe_int[0] * fringe_int[2] / 2)
            J2x = in_fringe * (K1 * fringe_int[2])
            J3x = in_fringe * (K1sqr * (fringe_int[4] - fringe_int[0] * fringe_int[1]))
            
            # For y-plane, use negative K1
            K1_y = -K1
            J1y = in_fringe * (K1_y * fringe_int[1] + K1_y * K1_y * fringe_int[0] * fringe_int[2])
            J2y = -J2x
            J3y = J3x
    ...
    def _apply_linear_quad_fringe_entrance(self, x: torch.Tensor, px: torch.Tensor, y: torch.Tensor, 
                                          py: torch.Tensor, tau: torch.Tensor, delta: torch.Tensor, 
                                          b2: float) -> None:
        """
        Apply quadrupole fringe field at entrance including linear Elegant-style effects.
    ...
    def _apply_linear_quad_fringe_exit(self, x: torch.Tensor, px: torch.Tensor, y: torch.Tensor, 
                                       py: torch.Tensor, tau: torch.Tensor, delta: torch.Tensor, 
                                       b2: float) -> None:
        """
