...
def create_pyat_quadrupole(length, k1, num_steps, fringe=0, radiation=False):
    ...
    pass_method = 'StrMPoleSymplectic4RadPass' if radiation else 'StrMPoleSymplectic4Pass'
    quad = at.Quadrupole('Q1', length, k1, NumIntSteps=num_steps, 
                         Energy=6.0e9, 
...
def create_pyat_sextupole(length, k2, num_steps, radiation=False):
    ...
    pass_method = 'StrMPoleSymplectic4RadPass' if radiation else 'StrMPoleSymplectic4Pass'
    sext = at.Sextupole('S1', length, k2, NumIntSteps=num_steps,
                        Energy=6.0e9,
                        PassMethod=pass_method)
    return sext
...
def create_pyat_dipole(length, k0, num_steps, radiation=False):
    ...
    dipole = at.Multipole('D1', length, poly_a, poly_b, 
...
def run_element_tracking(element_type, length, strength, num_steps, fringe=0, radiation=False):
    """Run tracking for a specific element type and return results for comparison"""
    # Create elements based on type
    if element_type == "drift":
        pyat_element = create_pyat_quadrupole(length, 0.0, num_steps, fringe, radiation)  # Drift is quad with k1=0
        cheetah_element = create_cheetah_quadrupole(length, 0.0, num_steps, fringe, radiation)
    elif element_type == "quadrupole":
        pyat_element = create_pyat_quadrupole(length, strength, num_steps, fringe, radiation)
        cheetah_element = create_cheetah_quadrupole(length, strength, num_steps, fringe, radiation)
    elif element_type == "sextupole":
        pyat_element = create_pyat_sextupole(length, strength, num_steps, radiation)
        cheetah_element = create_cheetah_sextupole(length, strength, num_steps, radiation)
        ...
        cheetah_element = create_cheetah_dipole(length, strength, num_steps, radiation=False)
    else:
    ...
    # Cheetah coordinates: [0]=x, [1]=px, [2]=y, [3]=py, [4]=tau, [5]=delta, [6]=1.0 (particle flag)
    cheetah_particle = torch.tensor([[X0, PX0, Y0, PY0, TAU0, DELTA0, 1.0]], dtype=torch.float64)
...
def assert_coordinates_close(pyat_final, cheetah_final, test_name):
    ...
    assert max_diff <= TOLERANCE, f"Coordinates differ by {max_diff} in {test_name} test"
...
def test_drift():
    ...
    # Verify that PyAT and Cheetah agree on the drift behavior by comparing their coordinate changes
    aligned_pyat = get_aligned_coordinates(pyat_final)
    
    # For drift, check that x and y changes match between PyAT and Cheetah
    assert abs((aligned_pyat[0] - X0) - (cheetah_final[0] - X0)) <= TOLERANCE, \
        "Drift x coordinate change doesn't match between PyAT and Cheetah"
    assert abs((aligned_pyat[2] - Y0) - (cheetah_final[2] - Y0)) <= TOLERANCE, \
...
def test_quadrupole_fringe_radiation():
    ...
    pyat_final, cheetah_final = run_element_tracking(
        "quadrupole", ELEMENT_LENGTH, QUAD_K1, NUM_STEPS, 
        fringe=FRINGE_ON, radiation=RADIATION_ON)
